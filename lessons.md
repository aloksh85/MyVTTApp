...
* **`pynput.keyboard.Controller.type()` on macOS:** Direct character-by-character keystroke simulation via `keyboard.type()` is unreliable on macOS. It requires Accessibility permissions that are silently revoked on every recompile (new binary signature), and even when granted, keystrokes can interleave with user input or be silently swallowed by the OS.
  * *Fix / Best Practice:* For text injection, use the **clipboard + simulated Cmd+V** approach (`pyperclip.copy()`).
* **Text Injection Permission Errors (macOS):** When Accessibility permission is revoked, `pynput` keystrokes fail silently. Using `osascript` to simulate keystrokes avoids silent failure but triggers a `1002` execution error because macOS treats command-line `osascript` as a separate, untrusted binary.
  * *Fix / Best Practice:* Use `pynput` to simulate `Cmd+V`, but explicitly guard it with a C-level Accessibility check using `ctypes.CDLL('/System/Library/Frameworks/ApplicationServices.framework/ApplicationServices').AXIsProcessTrusted()`. If this returns `False`, trigger a native UI dialog using `osascript -e 'display alert...'` and explicitly guide the user to System Settings. This ensures native apps never fail silently while still avoiding `osascript` keystroke blocks.
